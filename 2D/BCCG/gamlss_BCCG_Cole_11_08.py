...
def z_transform(y, x, x_LMS, L, M, S):
    ...
    #z = np.array([x for _,x in sorted(zip(x_LMS,z))])
    return z
    
#def y_transform(y, x, x_LMS):
#    y = np.zeros(len(x))
#    idx = [(np.abs(x[i] - x_LMS)).argmin() for i in range(0, len(y))]
#   
#    for i in range(0, len(L)):
...
while diff_LL>0.04:
    ...
    while diff_parameter>0.5:
        ...
        G_W = np.linalg.inv(W_L+alpha_L*K)
        G_M = np.linalg.inv(W_M+alpha_M*K)
        G_S = np.linalg.inv(W_S+alpha_S*K)

        L_calc = np.dot(G_W,(u_L+np.dot(W_L,L_0)-np.dot(W_LM, (M-M_0))-np.dot(W_LS, (S-S_0))))
        M_calc = np.dot(G_M,(u_M+np.dot(W_M,M_0)-np.dot(W_MS, (S-S_0))-np.dot(W_ML, (L-L_0))))
        S_calc = np.dot(G_S,(u_S+np.dot(W_S,S_0)-np.dot(W_SL, (L-L_0))-np.dot(W_SM, (M-M_0))))
